...
module.exports.authenticateUser = function (Models) {
    ...
    return async function (req, res, next) {
        ...
        try {
            ...
            if (req.url.indexOf('/survey/preview') === 0) {
                return next()
            }
            else if (req.url.indexOf('/survey/successPreview') === 0) {
                return next()
            }
            else if (req.url.indexOf('/survey/noPreview') === 0) {
                return next()
            }
            else if (req.url.indexOf('/users/login') === 0) {
